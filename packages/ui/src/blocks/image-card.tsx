...
function extractColors(imgEl: HTMLImageElement): RGB[] {
  try {
    ...
    return regions.map(({ x, y, w, h }) => {
      ...
      return n ? { r: Math.round(r/n), g: Math.round(g/n), b: Math.round(b/n) } : { r:100, g:100, b:100 }
    })
  } catch { return [{ r:80, g:80, b:120 }] }
}
...
const dk = (c: RGB, f: number): RGB => ({ r: Math.round(c.r*f), g: Math.round(c.g*f), b: Math.round(c.b*f) })
const rgb = (c: RGB) => `rgb(${c.r},${c.g},${c.b})`
const rgba = (c: RGB, a: number) => `rgba(${c.r},${c.g},${c.b},${a})`

function buildPalette(colors: RGB[]): Palette {
  const scored = colors
    .map(c => { const { s, l } = hsl(c.r, c.g, c.b); return { ...c, score: s * (1 - Math.abs(l-50)/50) } })
  ...
  const t = dk(tertiary,  0.30)
  const mid: RGB = { r: (p.r+s.r)>>1, g: (p.g+s.g)>>1, b: (p.b+s.b)>>1 }
...
const FALLBACK: Palette = {
  ...
  ambientGlow: "none",
  bottomFade:  "linear-gradient(to bottom, transparent 0%, rgba(22,33,62,0.75) 45%, rgba(22,33,62,0.98) 75%)",
...
const CardCtx = React.createContext<CardCtx>({
  palette: FALLBACK, setPalette: ()=>{},
  variant: "default", rounded: "default", size: "default",
})
 ...
 */
function buildGlowShadow(accentRgb: string, intensity: number = 1): string {
  return [
...
const imageCardVariants = cva(
  "group/image-card relative flex flex-col overflow-hidden text-white shadow-2xl transition-[box-shadow] duration-500",
  {
    ...
    defaultVariants: { variant: "default", rounded: "default", size: "default" },
  }
)
  ...
  return (
    <CardCtx.Provider value={{
      ...
      <div
        ...
        className={cn(imageCardVariants({ variant: resolvedVariant, rounded: resolvedRounded, size: resolvedSize }), className)}
...
export interface ImageCardImageProps extends React.ComponentPropsWithoutRef<"div"> {
  src:             string
  alt?:            string
...
export function ImageCardImage({
  className, src, alt = "", height = 240, objectPosition = "center top", children, ...props
}: ImageCardImageProps) {
  ...
  return (
    ...
    >
      <img
        ...
        crossOrigin="anonymous" onLoad={extract}
      ...
      <div aria-hidden className="pointer-events-none absolute inset-y-0 left-0  w-10 z-10"
        style={{ background: `linear-gradient(to right, ${palette.bottomBg}bb 0%, transparent 100%)` }} />
      <div aria-hidden className="pointer-events-none absolute inset-y-0 right-0 w-10 z-10"
        style={{ background: `linear-gradient(to left,  ${palette.bottomBg}bb 0%, transparent 100%)` }} />

      {/* Bottom gradient fade */}
      <div aria-hidden className="pointer-events-none absolute bottom-0 left-0 right-0 h-36 z-10"
        style={{ background: palette.bottomFade }} />

      {children && <div className="absolute inset-0 z-20 flex flex-col">{children}</div>}
    </div>
  )
}
...
export function ImageCardContent({ className, style, ...props }: React.ComponentPropsWithoutRef<"div">) {
...
const badgeVariants = cva(
  "inline-flex w-fit items-center font-semibold uppercase tracking-[0.15em] leading-none",
  {
    variants: {
      variant: {
        default: "text-white/70",
        accent:  "rounded-full px-2 py-0.5 text-white",
        outline: "rounded-full border border-white/30 px-2 py-0.5 text-white/80",
      },
...
export function ImageCardBadge({ className, variant = "default", size, style, ...props }: ImageCardBadgeProps) {
  const { palette, size: cs } = useCard()
  const sz = size ?? (cs as VariantProps<typeof badgeVariants>["size"])
  const accentStyle = variant === "accent"
    ? { background: `rgba(${palette.accentRgb},0.28)`, border: `1px solid rgba(${palette.accentRgb},0.38)` }
    : {}
  return (
    <span data-slot="image-card-badge"
      className={cn(badgeVariants({ variant, size: sz }), className)}
      style={{ ...accentStyle, ...style }} {...props} />
  )
}
...
export function ImageCardTitle({ className, ...props }: React.ComponentPropsWithoutRef<"h3">) {
  const { size } = useCard()
  return (
    <h3 data-slot="image-card-title"
      className={cn(
        "font-bold leading-tight text-white",
        size === "sm" ? "text-lg" : size === "lg" ? "text-3xl" : "text-[22px]",
        className,
      )}
      style={{ textShadow: "0 1px 8px rgba(0,0,0,0.4)", letterSpacing: "-0.3px" }}
      {...props} />
  )
}
...
export function ImageCardDescription({ className, ...props }: React.ComponentPropsWithoutRef<"p">) {
  return (
    <p data-slot="image-card-description"
      className={cn("text-white/60 leading-snug", className)} {...props} />
  )
}
...
export function ImageCardFooter({ className, style, ...props }: React.ComponentPropsWithoutRef<"div">) {
  const { palette, size } = useCard()
  return (
    <div data-slot="image-card-footer"
      className={cn(
        "relative z-10 flex items-center gap-3",
        size === "sm" ? "px-3 pt-2 pb-3" : size === "lg" ? "px-6 pt-3 pb-5" : "px-5 pt-3 pb-4",
        className,
      )}
      style={{
...
export function ImageCardAppIcon({ className, size, src, alt, style, children, ...props }: ImageCardAppIconProps) {
  const { palette, size: cs } = useCard()
  const sz = size ?? (cs as VariantProps<typeof appIconVariants>["size"])
  return (
    <div data-slot="image-card-app-icon"
...
export function ImageCardAppInfo({ className, ...props }: React.ComponentPropsWithoutRef<"div">) {
  return <div data-slot="image-card-app-info" className={cn("flex min-w-0 flex-1 flex-col gap-0.5", className)} {...props} />
}

export function ImageCardAppName({ className, ...props }: React.ComponentPropsWithoutRef<"p">) {
  return <p data-slot="image-card-app-name" className={cn("truncate font-semibold leading-tight text-white", className)} {...props} />
}

export function ImageCardAppDesc({ className, ...props }: React.ComponentPropsWithoutRef<"p">) {
  return <p data-slot="image-card-app-desc" className={cn("truncate text-[11px] leading-tight text-white/50", className)} {...props} />
}
...
const actionVariants = cva(
  "inline-flex shrink-0 cursor-pointer items-center justify-center font-semibold transition-all duration-150 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40",
  {
    variants: {
      variant: {
        default: "text-white",
...
export function ImageCardAction({ className, variant = "default", size, style, ...props }: ImageCardActionProps) {
  const { palette, size: cs } = useCard()
  const sz = size ?? (cs as VariantProps<typeof actionVariants>["size"])
  return (
    <button data-slot="image-card-action"
...
export { imageCardVariants, badgeVariants, actionVariants, appIconVariants }
