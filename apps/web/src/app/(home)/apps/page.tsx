...
const TEMPLATES: AppTemplate[] = [
	...
	},
	{
		...
		preview:
			"https://grafana.com/static/img/screenshots/grafana_8_dashboard.png",
		accentColor: "hsl(var(--primary))",
		stars: "64k",
	},
	{
...
function TagPill({
	...
	return (
		<button
			onClick={onClick}
			className={`inline-flex items-center rounded-full border px-2.5 py-0.5 font-medium text-[11px] transition-all ${active ? "border-primary bg-primary text-primary-foreground" : color}
...
function AppLogo({ src, name }: { src: string; name: string }) {
	...
	if (err) {
		return (
			<div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-primary/10 font-bold text-lg text-primary">
				{name[0]}
			</div>
		);
	}
		...
		>
			...
			>
				<button
					...
					className="flex items-center gap-1.5 rounded-xl bg-background px-4 py-2 font-semibold text-foreground text-xs shadow transition-colors hover:bg-muted"
				>
		...
		<Link href={`/apps/${t.slug}`} className="group block">
			<Card className="flex h-full flex-col overflow-hidden border-border bg-card transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
				...
				<div className="relative overflow-hidden">
					...
					<span className="absolute top-2.5 right-2.5 rounded-full border border-border bg-background/90 px-2 py-0.5 font-semibold text-[10px] text-foreground shadow-sm backdrop-blur-sm">
						{t.category}
					</span>
				</div>
					...
					<div className="flex flex-1 flex-col gap-3 p-4">
						<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
							...
							<div className="flex shrink-0 items-center gap-2 sm:flex-col sm:items-end">
								...
								<Button
									size="sm"
									className="h-7 gap-1 whitespace-nowrap rounded-lg px-3 text-[11px]"
								>
						...
						<p className="line-clamp-1 hidden text-muted-foreground text-xs leading-relaxed sm:block">
							{t.description}
						</p>
						...
						<div className="flex items-center gap-3">
							...
							<button
								...
								className="flex items-center gap-1 font-medium text-[11px] text-primary transition-colors hover:text-primary/80"
							>
		...
		<div className="flex flex-col gap-5">
			...
			<div>
				...
				<div className="mt-1.5 flex justify-between">
					<span className="text-muted-foreground text-xs">Rp 10.000</span>
		...
		<div className="bg-background">
			...
			<section className="border-b bg-linear-to-b from-primary/5 to-background">
				<div className="mx-auto max-w-5xl px-4 pt-32 pb-16 text-center">
					...
					<p className="mx-auto max-w-xl text-muted-foreground text-sm md:text-base">
					...
					<div className="mb-4 flex flex-wrap items-center gap-2">
						...
						{activeCategory !== "All" && (
							<button
								...
								className="flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 font-medium text-primary text-xs transition-colors hover:bg-primary/20"
							>
						...
						{activeTags.map((tag) => (
							<button
								key={tag}
								onClick={() => toggleTag(tag)}
								className="flex items-center gap-1 rounded-full bg-muted px-2.5 py-1 font-medium text-foreground text-xs transition-colors hover:bg-muted/80"
							>
