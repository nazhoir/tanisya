				...
				<div className="flex flex-col sm:flex-row">
					...
					<div className="flex min-w-0 flex-1 flex-col">
						<CardHeader className="px-4 pt-4 pb-2 sm:px-5 sm:pt-5">
							...
							<h3 className="break-words font-bold text-sm leading-snug transition-colors group-hover:text-primary sm:text-base">
								{post.title}
							</h3>
		...
		<Link href={`/blog/${post.slug}`} className="block h-full">
			<Card className="group flex h-full flex-col overflow-hidden border-border transition-shadow hover:shadow-lg">
				...
				<CardHeader className="px-4 pt-3 pb-2">
					...
					<h3 className="line-clamp-2 break-words font-bold text-sm leading-snug transition-colors group-hover:text-primary">
						{post.title}
					</h3>
					<p className="mt-1 line-clamp-2 break-words text-muted-foreground text-xs leading-relaxed">
						{post.excerpt}
				...
				<CardFooter className="mt-auto items-center justify-between px-4 py-3">
					...
					<span className="flex shrink-0 items-center gap-1 self-end font-medium text-primary text-xs">
						Baca
						<ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
					</span>
				</CardFooter>
	...
	return (
		...
		<div className="flex gap-1.5 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
			{CATEGORIES.map((c) => {
				...
				const count =
					c.value !== "all"
						? counts[c.value as Exclude<BlogCategory, "all">]
						: null;
				return (
					<button
						key={c.value}
						onClick={() => onChange(c.value)}
						className={`flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 font-medium text-xs transition-colors ${
							active
					...
					>
						...
						{count !== null && (
							<span
								className={`flex h-4 min-w-[1rem] items-center justify-center rounded-full px-1 font-bold text-[10px] ${
									active
	...
	return (
		...
		<div className="overflow-x-hidden bg-background">
			...
			<section className="border-b bg-linear-to-b from-primary/10 to-background">
				<div className="mx-auto max-w-5xl px-4 py-10 text-center sm:px-6 md:pt-32 md:pb-16">
					...
					<p className="mx-auto max-w-xl text-muted-foreground text-sm sm:text-base md:text-lg">
