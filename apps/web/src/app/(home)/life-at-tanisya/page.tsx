      ...
      <main className="w-full">
        ...
        <section className="relative flex min-h-[70svh] items-center">
          <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
            ...
            <div className="absolute right-0 top-1/4 h-40 w-40 rounded-full bg-violet-500/8 blur-[80px] sm:h-64 sm:w-64" />
          ...
          <div className="mx-auto w-full max-w-5xl px-4 pb-14 pt-24 sm:px-6 sm:pt-36">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="mb-7 max-w-sm text-sm leading-relaxed text-muted-foreground sm:max-w-xl sm:text-base lg:text-lg">
                Di Tanisya, kamu bukan sekadar karyawan — kamu adalah bagian dari tim kecil yang
                membangun infrastruktur digital untuk ribuan bisnis Indonesia. Remote-first,
                ownership tinggi, dan dampak nyata setiap harinya.
              </p>
              ...
              <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
                ...
                <Link href="/contact" className="w-full sm:w-auto">
                  ...
                  >
                    Kirim CV Spontan <MessageCircle className="h-4 w-4 shrink-0" />
                  </Button>
                </Link>
              </div>

              <p className="mt-3 text-xs text-muted-foreground">
                Tidak ada posisi cocok? · Kirim CV spontan · Kami selalu buka untuk talenta luar biasa
              </p>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6">
            <div className="mb-7 text-center sm:mb-12">
              ...
              <p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">
                Bukan sekadar gaji — kami memastikan kamu punya semua yang dibutuhkan
                untuk berkembang dan bekerja dengan bahagia.
              </p>
            </div>
        ...
        {/* ── JOB LISTINGS ─────────────────────────────────────────────────── */}
        <section id="lowongan" className="border-t border-border/60 bg-muted/20 py-12 sm:py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-4 sm:px-6">
            {/* Section header */}
            <div className="mb-7 flex flex-col gap-4 sm:mb-10 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2 sm:justify-end">
                {DEPARTMENTS.map(({ icon: Icon, label, count }) => (
                  <div
                    key={label}
                    className="flex items-center gap-1.5 rounded-full border border-border/60 bg-background px-3 py-1.5 text-xs font-semibold transition-all hover:border-primary/40 hover:bg-primary/5 cursor-pointer"
                  >
            ...
            {(["Engineering", "Design", "Product", "Marketing", "Customer Success"] as const).map(
              (dept) => {
                const jobs = JOB_LISTINGS.filter((j) => j.department === dept);
                if (!jobs.length) return null;
                const DeptIcon = DEPARTMENTS.find((d) => d.label === dept)?.icon ?? Briefcase;
                return (
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {tags.map((tag) => (
                                  <span
                                    key={tag}
                                    className="rounded-full border border-border/60 bg-muted/50 px-2.5 py-1 text-[11px] font-semibold text-muted-foreground"
                                  >
                                    {tag}
                                  </span>
                                ))}
            ...
            <div className="relative">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5 lg:gap-0">
                {PROCESS.map(({ step, title, description }, i) => (
                  <div key={step} className="relative flex flex-col items-start gap-3 lg:items-center lg:px-4 lg:text-center">
                    {/* Mobile: left border line */}
                    {i < PROCESS.length - 1 && (
                      <div className="absolute left-5 top-12 h-full w-px bg-border/60 sm:hidden" />
                    )}
              ...
              <div className="relative flex flex-col gap-7 p-6 sm:p-10 lg:flex-row lg:items-center lg:gap-12 lg:p-16">
                <div className="flex-1">
                  <div className="mb-4 inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold">
                    <Rocket className="h-3.5 w-3.5 shrink-0" /> TIDAK ADA POSISI YANG COCOK?
                  </div>
                  ...
                  <p className="mb-6 text-sm leading-relaxed text-primary-foreground/80 sm:text-base">
                    Kami selalu tertarik bertemu dengan orang-orang luar biasa — bahkan sebelum ada
                    posisi terbuka. Kirim CV dan ceritakan apa yang ingin kamu bangun bersama kami.
                  </p>
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <Link href="/contact?ref=spontaneous" className="w-full sm:w-auto">
                      <Button
                        size="lg"
                    ...
                    <a href="mailto:careers@tanisya.com" className="w-full sm:w-auto">
                      <Button
                        size="lg"
                        variant="ghost"
                        className="h-12 w-full gap-2 border border-white/20 text-primary-foreground hover:bg-white/10 sm:w-auto"
                      >
                        careers@tanisya.com <MessageCircle className="h-3.5 w-3.5" />
                      </Button>
                    </a>
                  </div>
                </div>
        ...
        <section className="border-t border-border/60 py-14 sm:py-24">
          <div className="mx-auto max-w-2xl px-4 text-center sm:px-6">
            ...
            <p className="mb-8 text-sm text-muted-foreground sm:text-base">
              Gabung dengan tim yang peduli pada craft, kualitas, dan dampak nyata
              untuk ekosistem digital Indonesia.
            </p>
