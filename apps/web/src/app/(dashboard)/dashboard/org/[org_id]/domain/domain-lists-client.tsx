...
function StatsCards() {
	...
	const expired = domains.filter(
		(d) => d.status === "expired" || d.status === "suspended",
	).length;
	...
	return (
		<div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border/60 bg-border/25 sm:grid-cols-4">
			{[
				...
				<div
					key={label}
					className="flex items-center gap-3 bg-background p-4 sm:p-5"
				>
					<div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10">
						<Icon className="h-4 w-4 text-primary" />
					</div>
					<div>
						<p className="font-extrabold text-2xl leading-none tracking-tight">
							{value}
						</p>
...
export function DomainClient({ orgId }: { orgId: string }) {
	...
	const columns: ColumnDef<Domain>[] = [
		{
			...
			cell: ({ row }) => {
				...
				return (
					<Link
						href={`${baseUrl}/domain/${name}${tld}` as any}
					...
					>
						...
						<div className="min-w-0">
							<p className="truncate font-semibold text-sm transition-colors group-hover:text-primary">
								{name}
								<span className="font-normal text-muted-foreground">{tld}</span>
							</p>
						</div>
				...
				return (
					<div className="flex flex-col gap-0.5">
						<span className="text-sm">
							{formatDate(row.getValue("nextDue"))}
						</span>
						{days > 0 && days <= 30 && (
						...
						{days <= 0 && (
							<span className="text-destructive text-xs">Sudah berakhir</span>
						)}
					</div>
				);
			},
					...
					<div className="flex justify-end gap-2">
						{row.original.status === "expiring" && (
							<Button
								...
								asChild
							>
								<Link href={`${baseUrl}/domain/renew?domain=${full}` as any}>
						...
						<Button size="sm" variant="ghost" className="h-7 text-xs" asChild>
							<Link href={`${baseUrl}/domain/${full}` as any}>Kelola</Link>
						</Button>
					</div>
				);
			},
		...
		<>
			<div className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
				<div className="flex items-center justify-between">
					...
					<Button
						className="h-9 gap-2 font-semibold shadow-primary/20 shadow-sm"
						asChild
					>
						<Link href={`${baseUrl}/domain/register` as any}>
				...
				<div className="rounded-2xl border border-border/60 bg-background">
					{/* Toolbar */}
					<div className="flex items-center justify-between gap-3 border-border/60 border-b p-4">
							...
							{table.getRowModel().rows.length ? (
								table.getRowModel().rows.map((row) => (
									<TableRow
										...
										onClick={() => {
											const { name, tld } = row.original;
											window.location.href = `${baseUrl}/domain/${name}${tld}`;
										}}
									>
									...
									<TableCell colSpan={columns.length} className="p-0">
										<Empty className="rounded-none border-none py-12">
											<EmptyHeader>
												...
												<EmptyDescription>
													Kamu belum memiliki domain yang terdaftar. Daftarkan
													domain pertamamu sekarang.
												</EmptyDescription>
											</EmptyHeader>
											<EmptyContent>
