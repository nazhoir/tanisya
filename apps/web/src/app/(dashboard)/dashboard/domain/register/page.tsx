...
const TLD_GROUPS = [
  {
    label: "Indonesia",
    tlds: [".id", ".co.id", ".my.id", ".web.id", ".biz.id", ".or.id", ".ac.id", ".sch.id", ".ponpes.id"],
  },
  {
    label: "Global",
    tlds: [".com", ".net", ".org", ".info", ".biz", ".io", ".co", ".xyz", ".online", ".site", ".store", ".tech"],
  },
];
...
function applySort(items: TldResult[], mode: SortMode): TldResult[] {
  ...
  const sorted = [...avail].sort((a, b) => {
    const aPrice = TLD_PRICES[a.tld]?.reg
      ? parseInt(TLD_PRICES[a.tld].reg.replace(/\D/g, ""), 10) : Infinity;
    const bPrice = TLD_PRICES[b.tld]?.reg
      ? parseInt(TLD_PRICES[b.tld].reg.replace(/\D/g, ""), 10) : Infinity;
...
/**
 * Deteksi apakah input adalah mode batch:
 * - mengandung koma DAN setiap item punya titik → batch
 * - selain itu → by-extensions
 */
...
/**
 * Tombol transfer domain — ditampilkan saat domain tidak tersedia (sudah terdaftar).
 * Mengarahkan user ke halaman transfer dengan domain pre-filled.
 */
function TransferButton({ domainName }: { domainName: string }) {
  return (
    <Button
      size="sm"
      variant="outline"
      className="h-8 gap-1.5 text-xs font-semibold border-amber-400/50 text-amber-600 hover:bg-amber-50 hover:text-amber-700 hover:border-amber-400 dark:border-amber-500/40 dark:text-amber-400 dark:hover:bg-amber-950/30 dark:hover:text-amber-300"
      asChild
    >
      <Link href={`/dashboard/domain/transfer?domain=${encodeURIComponent(domainName)}`}>
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <span className={cn("text-sm font-semibold", !available && "opacity-60")}>
            {domainName}
          </span>
          <span className={cn("text-sm text-muted-foreground", !available && "opacity-60")}>
            {result.tld}
          </span>
          {POPULAR_TLDS.includes(result.tld) && (
            <Badge variant="outline" className="ml-2 h-4 border-primary/30 bg-primary/8 px-1.5 text-[10px] text-primary">
              Populer
            </Badge>
          )}
          {!available && (
            <Badge variant="outline" className="ml-2 h-4 border-amber-400/40 bg-amber-50 px-1.5 text-[10px] text-amber-600 dark:bg-amber-950/30 dark:text-amber-400">
              Sudah terdaftar
            </Badge>
          )}
        </div>
          ...
          {available && (
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold">
                {result.price}
                <span className="text-muted-foreground font-normal">/thn</span>
              </p>
              <p className="text-xs text-muted-foreground">Perpanjang {result.priceRenew}</p>
            </div>
          )}
          ...
          {available ? (
            <Button
              ...
              className={cn(
                ...
                inCart && "border-primary/30 text-primary hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30",
              )}
            ...
            <TransferButton domainName={result.name} />
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="rounded-2xl border border-primary/30 bg-primary/5 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-primary">Keranjang ({cart.length})</p>
      </div>

      <div className="space-y-1.5">
        {cart.map((item) => (
          <div key={`${item.domain}${item.tld}`} className="flex items-center justify-between text-sm">
            <span>
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm text-muted-foreground">Total per tahun</span>
        <span className="font-extrabold">Rp {total.toLocaleString("id-ID")}</span>
      </div>

      <Button className="w-full gap-2 font-bold shadow shadow-primary/20" asChild>
        <Link href="/dashboard/checkout/domain">
...
export default function DomainRegisterPage() {
  ...
  // ── Search mode ──────────────────────────────────────────────────────────
  const [searchMode, setSearchMode] = React.useState<SearchMode>("by-extensions");
  ...
  // Cart: { domain, tld, price }[]
  const [cart, setCart] = React.useState<{ domain: string; tld: string; price: string }[]>([]);

  const inputRef = React.useRef<HTMLInputElement>(null);

  // ── Mutation: checkByExtensions ──────────────────────────────────────────
  const byExtMutation = useMutation(
    orpc.domain.checkByExtensions.mutationOptions({
      onSuccess: (data: { baseName: string; results: DomainSuggestion[] }) => {
        ...
        const firstAvailable = sortedRest.find((r) => r.available) ?? sortedRest[0];
  ...
  const runByExtensions = React.useCallback((rawInput: string, exts?: string) => {
  ...
  const handleSearch = () => {
    if (searchMode === "batch") {
      ...
      router.replace(`/dashboard/domain/register?domain=${encodeURIComponent(batchQuery.trim())}&mode=batch`);
      runBatch(batchQuery);
    } else {
      const raw = query.trim().replace(/^https?:\/\//, "");
      if (!raw || byExtMutation.isPending) return;
      router.replace(`/dashboard/domain/register?domain=${encodeURIComponent(raw)}`);
      runByExtensions(raw);
    }
  };
  ...
  const handleLoadMore = () => {
    const baseName = domainName;
    if (!baseName || secondaryMutation.isPending) return;
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        {/* ── Content ────────────────────────────────────────────────────── */}
        <div className="flex flex-1 flex-col gap-6 p-4 pt-0 sm:p-6 sm:pt-0 max-w-5xl">
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className={cn(
                      "h-12 w-full rounded-xl border border-border bg-background pl-11 pr-4 text-sm font-medium shadow-sm transition-all",
                  ...
                  {query && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSecondaryList([]);
                        setHasSearched(false);
                        router.replace("/dashboard/domain/register");
                ...
                {/* Ekstensi picker toggle */}
                <Button
                  variant="outline"
                  size="sm"
                  className={cn(
                    "h-12 gap-1.5 rounded-xl px-4 text-sm font-semibold",
                    showTldPicker && "border-primary/40 bg-primary/5 text-primary",
                  )}
                  onClick={() => setShowTldPicker((v) => !v)}
                >
                  <Globe className="h-4 w-4" />
                  <span className="hidden sm:inline">Ekstensi</span>
                  <Badge variant="outline" className="border-current bg-transparent px-1.5 text-xs">
                    {selectedTlds.length}
                  </Badge>
                </Button>
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (query.trim()) runByExtensions(query, selectedTlds.join(","));
                      }}
                    >
                      Terapkan
                    </Button>
          ...
          {searchMode === "batch" && (
            <div className="flex gap-2">
              <div className="relative flex-1">
                <List className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
                <input
                  type="text"
                  value={batchQuery}
          ...
          {!isLoading && hasSearched && searchMode === "by-extensions" && (exactDomain ?? topRec) && (
            <div className="grid gap-6">
              <div className="space-y-6">

                {/* Exact + top recommendation */}
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                        Domain Lainnya
                        <Badge variant="secondary" className="h-4 px-1.5 text-[10px] normal-case tracking-normal">
              ...
              <CartSummary cart={cart} onRemove={(key) =>
                setCart((prev) => prev.filter((i) => `${i.domain}${i.tld}` !== key))
              } />
            </div>
          )}

          {/* ── Results: batch ────────────────────────────────────────────── */}
          {!isLoading && hasSearched && searchMode === "batch" && batchResults.length > 0 && (
            <div className="grid gap-6">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          ...
          {!isLoading && !hasSearched && (
            <div className="rounded-2xl border border-dashed border-border/60 bg-muted/10 px-6 py-12 text-center">
              ...
              <p className="font-semibold">
                {searchMode === "batch" ? "Cek banyak domain sekaligus" : "Cari nama domain kamu"}
              </p>
