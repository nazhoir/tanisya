...
function TransferButton({ domainName }: { domainName: string }) {
	return (
		...
		>
			<Link
				href={`/dashboard/domain/transfer?domain=${encodeURIComponent(domainName)}`}
			>
...
function CartSummary({
	...
	const total = cart.reduce((sum, item) => {
		return sum + Number.parseInt(item.price.replace(/\D/g, ""), 10);
	}, 0);
...
export default function DomainRegisterPage() {
	...
	const handleSearch = () => {
		if (searchMode === "batch") {
			...
			router.replace(
				`/dashboard/domain/register?domain=${encodeURIComponent(batchQuery.trim())}&mode=batch`,
			);
			runBatch(batchQuery);
		} else {
			...
			router.replace(
				`/dashboard/domain/register?domain=${encodeURIComponent(raw)}`,
			);
			runByExtensions(raw);
		}
	};
	...
	return (
		<SidebarProvider>
			...
			<SidebarInset>
					...
					<div className="flex items-center gap-2 px-4">
						...
						<Breadcrumb>
							<BreadcrumbList>
								...
								<BreadcrumbItem className="hidden md:block">
									<BreadcrumbLink href="/dashboard/domain">
										Domain
									</BreadcrumbLink>
								</BreadcrumbItem>
								<BreadcrumbSeparator className="hidden md:block" />
				...
				{/* ── Content ────────────────────────────────────────────────────── */}
				<div className="flex max-w-5xl flex-1 flex-col gap-6 p-4 pt-0 sm:p-6 sm:pt-0">
					{/* Back + heading */}
					<div>
						<Button
							variant="ghost"
						...
						<p className="mt-0.5 text-muted-foreground text-sm">
							Cari nama domain yang kamu inginkan dan pilih ekstensi yang
							sesuai.
						</p>
					</div>
