    ...
    onSubmit: async ({ value }) => {
      await authClient.emailOtp.verifyEmail(
        { email, otp: value.otp },
        {
          ...
          onError: (error) => {
            toast.error(error.error.message || "Kode OTP tidak valid atau sudah kadaluarsa");
          },
        },
      );
    },
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      ...
      <div className="w-full max-w-md">
        {/* Card */}
        <div className="overflow-hidden rounded-2xl border border-border/60 bg-background shadow-xl shadow-black/5">
          {/* Top gradient bar */}
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="mb-6 flex flex-col items-center text-center">
              ...
              <div className="mt-1.5 flex items-center gap-1.5 rounded-full border border-border/60 bg-muted/50 px-3 py-1">
                ...
                <span className="max-w-[220px] truncate text-xs font-semibold">{email}</span>
              </div>
            </div>
              ...
              <FieldGroup>
                ...
                <form.Field
                  ...
                  children={(field) => {
                    ...
                    const otpDigits = field.state.value.split("").concat(Array(6).fill("")).slice(0, 6);
                        ...
                        >
                          {otpDigits.map((digit, index) => (
                            <input
                              ...
                              onKeyDown={(e) =>
                                handleOtpKeyDown(index, e, field.state.value, field.handleChange)
                              }
                              onFocus={(e) => e.target.select()}
                              className={cn(
                                "h-12 w-10 rounded-xl border bg-background text-center text-xl font-extrabold tracking-widest outline-none transition-all sm:h-14 sm:w-12 sm:text-2xl",
        ...
        <p className="mt-4 px-4 text-center text-xs text-muted-foreground">
          Kode OTP berlaku selama <span className="font-semibold text-foreground">10 menit</span>.
