...
function NavBadge({ label }: { label: string }) {
	return (
		<Badge
			className={cn(
				"h-4 shrink-0 px-1.5 py-0 font-bold text-[9px]",
	...
	return (
		...
		>
			...
			<span className="min-w-0 flex-1">
				...
				<span className="line-clamp-1 block text-[11px] text-muted-foreground leading-snug">
					{description}
				</span>
			</span>
		</Link>
...
function HighlightedCard({
	...
	return (
		<Link
			href={to as any}
			className="group relative flex h-full flex-col overflow-hidden rounded-2xl bg-linear-to-br from-primary via-primary/90 to-primary/60 p-5 text-primary-foreground transition-all hover:shadow-primary/20 hover:shadow-xl"
		>
			{/* decorative blobs */}
			<span className="pointer-events-none absolute -top-6 -right-6 h-24 w-24 rounded-full bg-white/10 blur-2xl" />
			...
			<span className="relative mb-auto">
				...
				<span className="mb-1 flex items-center gap-2">
					...
					{badge && (
						<span className="rounded-full bg-white/25 px-2 py-0.5 font-bold text-[9px] tracking-wide">
							{badge}
						</span>
					)}
				</span>
			...
			<span className="relative mt-4 inline-flex items-center gap-1.5 font-bold text-xs transition-all group-hover:gap-2.5">
...
function ProductPanel() {
	...
	const highlighted = PRODUCT_ITEMS.flatMap((c) => c.items).find(
		(i) => i.isHiglighted,
	);
	...
	return (
		<div className="flex min-h-80 gap-0">
			{/* Left: category tabs */}
			<div className="mr-4 flex w-56 shrink-0 flex-col gap-1 border-border/60 border-r pr-4">
				...
				{PRODUCT_ITEMS.map((cat, idx) => {
					...
					return (
						<button
							...
							className={cn(
								"flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-left font-semibold text-sm outline-none transition-all hover:bg-accent",
...
function DropdownPanel({
	...
	return (
		<div
			aria-hidden={!open}
			className={cn(
				"absolute top-full left-0 z-50 w-screen border-border/60 border-b bg-background",
...
function NavTrigger({
	...
	return (
		<button
			...
			className={cn(
				"inline-flex h-9 items-center gap-1 rounded-md px-4 py-2 font-medium text-sm outline-none transition-colors",
...
function MobileSectionLabel({ children }: { children: React.ReactNode }) {
	return (
		<p className="mt-4 mb-1 px-2 font-bold text-[10px] text-muted-foreground uppercase tracking-widest">
			{children}
		</p>
	);
}
...
function PromoPill() {
	return (
		<Link
			href="/promo"
			className="group relative hidden items-center gap-1.5 overflow-hidden rounded-full border border-primary/30 bg-primary/5 px-3.5 py-1.5 font-bold text-primary text-xs transition-all hover:border-primary/60 hover:bg-primary/10 xl:inline-flex"
		>
			{/* shimmer sweep */}
				...
				<div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-2">
					...
					<nav className="hidden flex-1 items-center justify-center gap-0.5 lg:flex">
						<Link
							href="/"
							className={cn(
								"inline-flex h-9 items-center rounded-md px-4 py-2 font-medium text-sm transition-colors hover:bg-accent hover:text-accent-foreground",
					...
					<DropdownPanel open={openMenu === "opensource"}>
						<div className="grid grid-cols-[1fr_auto] gap-8">
							...
							<div>
								...
								<Link
									href="/oss"
									className="mt-3 flex items-center gap-1 px-2 font-semibold text-[11px] text-primary hover:underline"
								>
							...
							<div className="w-52 shrink-0">
								<div className="rounded-2xl border border-violet-200 bg-violet-50 p-4 dark:border-violet-900 dark:bg-violet-950/40">
									...
									<Link
										...
										className="inline-flex items-center gap-1 font-semibold text-violet-600 text-xs hover:underline"
									>
